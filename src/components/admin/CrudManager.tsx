    ...
    mutationFn: async (row: Partial<Row>) => {
      if (row.id) {
        ...
        const { data, error } = await (supabase.from(table) as any)
          .insert({ ...row, sort_order: maxOrder + 1 })
          .select()
          .single();
        if (error) throw error;
        await logAudit("create", table, data?.id, {
          title: row[fields[0].key as keyof typeof row],
        });
      }
    },
    onSuccess: () => {
  ...
  const toggleActive = useMutation({
    mutationFn: async (r: Row) => {
      const { error } = await (supabase.from(table) as any)
        ...
        .eq("id", r.id);
      if (error) throw error;
      await logAudit("toggle_active", table, r.id, {
        title: r[fields[0].key],
      ...
      {isLoading ? (
        ...
        <div className="grid gap-3">
          {rows.map((r, i) => (
            <div
              key={r.id}
              className="bg-card rounded-xl border border-border p-4 flex flex-col sm:flex-row gap-3 items-start sm:items-center"
            >
              {fields.some((f) => f.type === "image") &&
                r[fields.find((f) => f.type === "image")!.key] && (
                  <img
                    src={r[fields.find((f) => f.type === "image")!.key]}
                    alt=""
              ...
              <div className="flex items-center gap-1 flex-wrap">
                ...
                <Button
                  ...
                  variant="ghost"
                  disabled={i === 0}
                  onClick={() => moveMut.mutate({ id: r.id, dir: -1 })}
                >
                ...
                <Button
                  ...
                  variant="ghost"
                  onClick={() => {
                    if (confirm("Удалить?")) delMut.mutate(r.id);
                  }}
                >
              ...
              <div key={f.key}>
                ...
                {f.type === "textarea" ? (
                  ...
                  <Input
                    ...
                    onChange={(e) => setEditing({ ...editing!, [f.key]: e.target.value })}
                    placeholder={
                      f.type === "image" ? "/images/file.jpg или https://..." : undefined
                    }
                  />
                )}
              </div>
