...
export const Route = createFileRoute("/admin/users")({
  head: () => ({
    meta: [{ title: "Пользователи — Админ" }, { name: "robots", content: "noindex" }],
  }),
  component: UsersPage,
});
...
function UsersPage() {
  ...
  const setRole = useMutation({
    mutationFn: async ({
      ...
      if (error) throw error;
      await logAudit(grant ? "role_grant" : "role_revoke", "user", target, {
        email,
        role: "admin",
      });
    },
              ...
              >
                <div className="flex-1 min-w-0">
                  <div className="font-medium truncate">
                    {u.email}
                    {isSelf && <span className="ml-2 text-xs text-muted-foreground">(вы)</span>}
                  </div>
                  <div className="text-xs text-muted-foreground">
                    Зарегистрирован: {new Date(u.created_at).toLocaleString("ru")}
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  ...
                  <Switch
                    ...
                    onCheckedChange={(v) =>
                      setRole.mutate({ target: u.user_id, grant: v, email: u.email })
                    }
                  />
                </div>
              </div>
